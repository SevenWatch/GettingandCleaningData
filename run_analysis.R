...
melted2 <- ddply(melted, c("ActivityLabel", "subjectID", "variable"), summarise,
                 mean = mean(value))
##This is the long format. However since many downgraded me.
##Here we change it to wide format. 
##The new format follows the one observation per row.
library(tidyr)

spreadtidydata <- melted2 %>% spread(variable, mean)
##Now we have to write the table. Don't forget row.names=FALSE

write.table(spreadtidydata, file= "./data/newUCIHARdataset.txt", row.names=FALSE)
